...
pub struct Withdraw<'info> {
    ...
    #[account(
        mut,
        seeds = [b"teasury", mint.key().as_ref()],
        bump,
    )]
    ...
    //the user should already have associated token account because they deposited to the lending protocol
    #[account(
        init_if_needed,
        payer = signer,
...
pub fn process_withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()>{
    ...
    let value_per_share = bank.total_deposits as f64 / bank.total_deposited_shares as f64;
    ...
    //we are transfering from bank token account which
    //has its own PDA, to transfer from account which has its own
    // pda, pda has to sign it and we need to derive signer seeds for it
    ...
    let bank = &mut ctx.accounts.bank;
    let shares_to_remove = (amount as f64 / bank.total_deposits as f64) * bank.total_deposited_shares as f64;
