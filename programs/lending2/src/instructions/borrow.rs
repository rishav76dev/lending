...
//challenge: to integrate dynamic interest rate
#[derive(Accounts)]
pub struct Borrow<'info>{
  ...
  // user is borrowing a new token and we don't if
  // they already have a token account for this token(sol,usdc)
  #[account(
      init_if_needed,
      payer = signer,
...
pub fn process_borrow(ctx: Context<Borrow>, amount: u64) -> Result<()>{
  ...
  match ctx.accounts.mint.to_account_info().key() {
    key if key == user.usdc_address => {
      ...
      let accrued_interest = calculate_accured_interest(user.deposited_sol, bank.interest_rate, user.last_update)?;
      total_collateral = sol_price.price as u64 * (user.deposited_sol + accrued_interest);
    },
    _ => {
      let usdc_feed_id = get_feed_id_from_hex(USDC_USD_FEED_ID)?;
      let usds_price = price_update.get_price_no_older_than(&Clock::get()?, MAX_AGE, &usdc_feed_id)?;

      let accrued_interest = calculate_accured_interest(user.deposited_usdc, bank.interest_rate, user.last_update)?;
      total_collateral = usds_price.price as u64 * accrued_interest;
    }
  }
