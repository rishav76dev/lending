...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()> {
  ...
  match ctx.accounts.collateral_mint.to_account_info().key(){
    key if key == user.usdc_address => {
      let new_usdc = calculate_accured_interest(user.deposited_usdc, collateral_bank.interest_rate, user.last_update)?;
      total_collateral = usdc_price.price as u64 * new_usdc;
      let new_sol = calculate_accured_interest(user.borrowed_sol, borrowed_bank.interest_rate, user.last_updated_borrowed)?;
      total_borrowed = sol_price.price as u64 * new_sol;
    }
    _ => {
      let new_sol = calculate_accured_interest(user.deposited_sol, collateral_bank.interest_rate, user.last_update)?;
      total_collateral = sol_price.price as u64 * new_sol;
      let new_usdc = calculate_accured_interest(user.borrowed_usdc, borrowed_bank.interest_rate, user.last_updated_borrowed)?;
      total_borrowed = usdc_price.price as u64 * new_usdc;
    }
  }

  let health_factor = (( total_collateral as f64 * collateral_bank.liquidation_threshold as f64) / total_borrowed as f64) as u64;

  if health_factor >= 1 {
    return Err(LiquidationError::NotUnderCollateralized.into());
  }
  ...
  token_interface::transfer_checked(cpi_ctx, total_borrowed, decimals)?;
  ...
  let transfer_to_liquidator = TransferChecked {
    ...
    authority: ctx.accounts.borrowed_bank_token_account.to_account_info(),
  ...
  let cpi_ctx_to_liquitor = CpiContext::new(cpi_program, transfer_to_liquidator).with_signer(signer_seeds);

  let collateral_decimals = ctx.accounts.collateral_mint.decimals;

  token_interface::transfer_checked(cpi_ctx_to_liquitor, liquidator_amount, collateral_decimals)?;
  Ok(())
}
